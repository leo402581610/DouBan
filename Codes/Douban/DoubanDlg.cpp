...
//last modify:	2013-01-18 10:41:48
//description:	source file for DoubanDlg.h
...
BOOL CDoubanDlg::OnSetCursor( CWnd* pWnd, UINT nHitTest, UINT message )
{
	if(IDC_TEXT_PREVIOUS == pWnd->GetDlgCtrlID() || IDC_TEXT_NEXT == pWnd->GetDlgCtrlID())
	{
		m_cur = LoadCursor(NULL, IDC_HAND);
		SetCursor(m_cur);
	}
	else
	{
		m_cur = LoadCursor(NULL, IDC_ARROW);
		SetCursor(m_cur);
	}
